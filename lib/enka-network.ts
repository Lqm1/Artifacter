...
const EnkaNetworkClient = ky.create({
  prefixUrl: "https://enka.network/",
  headers: {
    "User-Agent": "Artifacter",
  },
});

export const uidTest = (uid: string | number | undefined) =>
  /^(18|[1-35-9])\d{8}$/.test(uid?.toString() || "");

export const fetchUid = (uid: number): Promise<EnkaApi> =>
  EnkaNetworkClient.get(`api/uid/${uid}`).json<EnkaApi>();

export const avatarRemap = (info: AvatarInfo) => {
  const data = avatarJson.find(
    (e) => e.id === info.avatarId && e.skillDepotId === info.skillDepotId
  );
  ...
  const sideIcon = costume
    ? `UI_AvatarIcon_Side_${costume.key}`
    : `UI_AvatarIcon_Side_${data?.key}`;
  const avatarImg = costume
    ? `UI_Costume_${costume.key}`
    : `UI_Gacha_AvatarImg_${data?.key}`;
  const talentIcons =
    data?.consts.map((icon, i) => ({
      icon,
      unlock: i < (info.talentIdList?.length || 0),
    })) ||
    Array(6)
      .fill(0)
      .map(() => ({ icon: null, unlock: false }));
  const skills = (
    JSON.parse(JSON.stringify(data?.skills)) as
      | (typeof avatarJson)[number]["skills"]
      | undefined
  )?.map((skill) => ({
    ...skill,
  ...
  for (const equip of re.equipList || []) {
    if ("reliquary" in equip) {
      ...
      for (const sub of eq.flat.reliquarySubstats || []) {
        ...
        sub.rolls =
          rollSet.find((e) => e.prop === sub.appendPropId)?.rolls || [];
        // reliquarySubStats All
        const subStat = re.reliquarySubStats.find(
          (e) => e.appendPropId === sub.appendPropId
        );
        if (subStat) {
  ...
  re.reliquarySubStats.sort(
    (a, b) => getPropNumber(a.appendPropId) - getPropNumber(b.appendPropId)
  );
...
const get_character_stats = (avatarInfo: AvatarInfo) => {
  ...
  const stats = displayStatus.map((type) => {
    ...
    if (!fightProp)
      return { type, icon: "", value: { main: 0 }, display: { main: "" } };
  ...
  // dmg
  const dmgValues = displayDmgs.map(
    (e) =>
      avatarInfo.fightPropMap[
        fightProps.find((e2) => e2.type === e)?.prop || 0
      ] || 0
  );
...
const get_reliquary_sets = (avatarInfo: AvatarInfo) => {
  ...
  const uniqueId: string[] = [
    ...new Set(tmpList.map((e) => e.nameTextMapHash)),
  ];
  ...
  for (const e of tmpList) {
    setList[setList.findIndex((e2) => e2.nameTextMapHash === e.nameTextMapHash)]
      .count++;
  }
  const reList: {
    icon: string;
  ...
  for (const e of setList) {
    ...
    if (e.count < 4)
      reList.push({
        icon: e.icon,
        count: 2,
        nameTextMapHash: e.nameTextMapHash,
      });
    else
      reList.push({
        icon: e.icon,
  ...
  return reList.sort(
    (a, b) => Number(a?.icon?.slice(-7, -2)) - Number(b?.icon?.slice(-7, -2))
  );
};
const get_reliquary_roll_set = (idList: number[] | undefined) => {
  ...
  for (const id of idList || []) {
    const data = reliquaryAffixJson.find((e) => e.id === id);
    if (!data) continue;
