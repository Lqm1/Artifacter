import type { ReliquaryRemap } from "@/types/enka-network";
...
export const src = (
  ...
  folder?: "ui" | "element" | "build-card-assets",
  type?: "webp" | "png" | "jpg"
) => {
  if (folder === "ui")
...
export const elementColor = (
  ...
  light
    ? colorLight[element || ""] || colorLight.None
    : colorDark[element || ""] || colorDark.None;
...
export const getSubRollMark = (
  markProps: string[],
  reliquarySub: ReliquaryRemap["flat"]["reliquarySubstats"]
) => {
  const rollMap = reliquarySub?.map((sub) => ({
    appendPropId: sub.appendPropId,
    roll: sub.rolls.length,
  }));
  if (!rollMap) return 0;
  return rollMap
    .filter((e) => markProps.includes(e.appendPropId))
    .reduce((sum, e) => sum + e.roll, 0);
};
...
export const getScoreSet = (artifactList: ReliquaryRemap[], type: ScoreType) =>
  equipTypes.map((equipType) => {
    ...
    return {
      equipType: equipType,
      value: getScore(artifact, type),
      grade: getGrade(artifact, type, equipType),
    };
  });

export const getGrade = (
  artifact: ReliquaryRemap,
  type: ScoreType,
  equipType: string
) => {
  ...
  const tmpScore =
    score +
    (equipType === "EQUIP_BRACER" || equipType === "EQUIP_NECKLACE"
      ? 0
      : equipType === "EQUIP_SHOES" || equipType === "EQUIP_RING"
      ? 5
...
export const getScore = (artifact: ReliquaryRemap, type: ScoreType) => {
  ...
  for (const sub of artifact.flat.reliquarySubstats || []) {
    ...
    if (type === "EM" && name === "Elemental Mastery")
      score += sub.statValue / 4;
...
export const getTotalScoreSet = (
  ...
  const grade =
    tmpScore < 180 ? "B" : tmpScore < 200 ? "A" : tmpScore < 220 ? "S" : "SS";
  return { value, grade };
};
